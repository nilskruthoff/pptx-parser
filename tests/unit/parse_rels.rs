...
fn parses_slide_relationships_with_images() {
    let images =
        parse_slide_rels(&load_xml("rels_with_images.xml")).expect("parse image relationships");
    assert_eq!(images.len(), 2);
    assert_eq!(images[0].id, "rId1");
    assert_eq!(
        normalize_test_string(&images[0].target),
        normalize_test_string("../media/image1.png")
    );
    assert_eq!(images[1].id, "rId2");
    assert_eq!(
        normalize_test_string(&images[1].target),
        normalize_test_string("../media/image2.jpg")
    );
}
...
fn parses_hyperlink_relationships() {
    let hyperlinks = parse_hyperlink_rels(&load_xml("rels_with_images.xml"))
        .expect("parse hyperlink relationships");
    assert_eq!(
        hyperlinks.get("rId3"),
