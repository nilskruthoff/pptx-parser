...
fn test_parse_text() {
    ...
    match parse_text(&tx_body_node) {
        Ok(text_element) => {
            ...
            assert_eq!(
                normalize_test_string(&text_element.runs[0].text),
                normalize_test_string("Hello")
            );
            assert_eq!(
                normalize_test_string(&text_element.runs[1].text),
                normalize_test_string("World")
            );
            assert_eq!(
                normalize_test_string(&text_element.runs[2].text),
                normalize_test_string("!")
            );
        }
...
fn test_parse_run_with_format() {
    ...
    match parse_run(&r_node) {
        Ok(run) => {
            assert_eq!(
                normalize_test_string(&run.text),
                normalize_test_string("Formatted text")
            );
...
fn test_parse_run_no_format() {
    ...
    match parse_run(&r_node) {
        Ok(run) => {
            assert_eq!(
                normalize_test_string(&run.text),
                normalize_test_string("Unformatted text")
            );
            assert!(!run.formatting.bold);
            assert!(!run.formatting.italic);
            assert!(!run.formatting.underlined);
...
fn parses_run_hyperlink_target_from_relationships() {
    ...
    let hyperlinks =
        std::collections::HashMap::from([("rId7".to_string(), "https://example.com".to_string())]);
...
fn test_parse_paragraph_single() {
    ...
    match parse_paragraph(&p_node, true) {
        Ok(runs) => {
            ...
            assert_eq!(
                normalize_test_string(&runs[0].text),
                normalize_test_string("Single run\n")
            );
        }
...
fn test_parse_paragraph_multiple() {
    ...
    match parse_paragraph(&p_node, true) {
        Ok(runs) => {
            ...
            assert_eq!(
                normalize_test_string(&runs[0].text),
                normalize_test_string("First run")
            );
            assert_eq!(
                normalize_test_string(&runs[1].text),
                normalize_test_string("Second run")
            );
            assert_eq!(
                normalize_test_string(&runs[2].text),
                normalize_test_string("Third run\n")
            );
...
fn test_parse_list_properties_unordered() {
    ...
    match parse_list_properties(&p_node) {
        Ok((level, is_ordered)) => {
            ...
            assert!(
                is_ordered,
                "List should be identified as ordered due to buChar element"
            );
        }
...
fn test_parse_list_properties_ordered() {
    ...
    match parse_list_properties(&p_node) {
        Ok((level, is_ordered)) => {
            ...
            assert!(
                is_ordered,
                "List should be identified as ordered due to buAutoNum element"
            );
        }
...
fn test_parse_simple_list() {
    ...
    match parse_list(&tx_body_node) {
        Ok(list) => {
            ...
            assert!(
                list.items[0].is_ordered,
                "First item should be ordered (has buChar)"
            );
            assert_eq!(
                normalize_test_string(&list.items[0].runs[0].text),
                normalize_test_string("First item\n"),
            ...
            assert!(
                list.items[1].is_ordered,
                "Second item should be ordered (has buChar)"
            );
            assert_eq!(
                normalize_test_string(&list.items[1].runs[0].text),
                normalize_test_string("Second item\n"),
            ...
            assert!(
                list.items[2].is_ordered,
                "Third item should be ordered (has buChar)"
            );
            assert_eq!(
                normalize_test_string(&list.items[2].runs[0].text),
                normalize_test_string("Third item\n"),
...
fn test_parse_multilevel_list() {
    ...
    match parse_list(&tx_body_node) {
        Ok(list) => {
            ...
            assert_eq!(
                normalize_test_string(&list.items[0].runs[0].text),
                normalize_test_string("Main topic\n"),
                "First item text mismatch"
            );
            ...
            assert!(
                list.items[1].is_ordered,
                "Second item should be detected as ordered due to buChar"
            );
            assert_eq!(
                normalize_test_string(&list.items[1].runs[0].text),
                normalize_test_string("Subtopic bullet\n"),
                "Second item text mismatch"
            );
            ...
            assert_eq!(
                normalize_test_string(&list.items[3].runs[0].text),
                normalize_test_string("Numbered sub-subtopic\n"),
            ...
            assert_eq!(
                normalize_test_string(&list.items[4].runs[0].text),
                normalize_test_string("Second main topic\n"),
...
fn test_parse_table_cell_simple() {
    ...
    match parse_table_cell(&tc_node) {
        Ok(cell) => {
            ...
            assert_eq!(
                normalize_test_string(&cell.runs[0].text),
                normalize_test_string("Cell 1,1")
            );
        }
...
fn test_parse_table_cell_complex() {
    ...
    match parse_table_cell(&tc_node) {
        Ok(cell) => {
            ...
            assert_eq!(
                normalize_test_string(&cell.runs[0].text),
                normalize_test_string("Multiple")
            );
            assert_eq!(
                normalize_test_string(&cell.runs[1].text),
                normalize_test_string("paragraphs")
            );
            assert_eq!(
                normalize_test_string(&cell.runs[2].text),
                normalize_test_string("in one cell")
            );
        }
...
fn test_parse_table_row_simple() {
    ...
    match parse_table_row(&tr_node) {
        Ok(row) => {
            ...
            assert_eq!(
                normalize_test_string(&row.cells[0].runs[0].text),
                normalize_test_string("Cell 1,1")
            );
            assert_eq!(
                normalize_test_string(&row.cells[1].runs[0].text),
                normalize_test_string("Cell 1,2")
            );
        }
...
fn test_parse_table_row_complex() {
    ...
    let tr_node = doc
        .root_element()
        .descendants()
        .find(|n| n.is_element() && n.tag_name().name() == "tr") // Erste Zeile mit fetten Überschriften
...
fn test_parse_simple_table() {
    ...
    match parse_table(&tbl_node) {
        Ok(table) => {
            ...
            assert_eq!(
                normalize_test_string(&table.rows[0].cells[0].runs[0].text),
                normalize_test_string("Cell 1,1"),
                "Cell content mismatch"
            );
            assert_eq!(
                normalize_test_string(&table.rows[0].cells[1].runs[0].text),
                normalize_test_string("Cell 1,2"),
                "Cell content mismatch"
            );
            ...
            assert_eq!(
                normalize_test_string(&table.rows[1].cells[0].runs[0].text),
                normalize_test_string("Cell 2,1"),
                "Cell content mismatch"
            );
            assert_eq!(
                normalize_test_string(&table.rows[1].cells[1].runs[0].text),
                normalize_test_string("Cell 2,2"),
                "Cell content mismatch"
            );
        }
...
fn test_parse_complex_table() {
    ...
    match parse_table(&tbl_node) {
        Ok(table) => {
            ...
            for i in 0..3 {
                assert!(
                    table.rows[0].cells[i].runs[0].formatting.bold,
                    "Header cell should have bold formatting"
                );
                assert!(
                    normalize_test_string(&table.rows[0].cells[i].runs[0].text)
                        .starts_with("Heading"),
                    "Header should start with 'Heading'"
                );
            }
            ...
            assert_eq!(
                normalize_test_string(&table.rows[1].cells[0].runs[0].text),
                normalize_test_string("Multiple"),
                "First paragraph content mismatch"
            );
            assert_eq!(
                normalize_test_string(&table.rows[1].cells[0].runs[1].text),
                normalize_test_string("paragraphs"),
                "Second paragraph content mismatch"
            );
            assert_eq!(
                normalize_test_string(&table.rows[1].cells[0].runs[2].text),
                normalize_test_string("in one cell"),
                "Third paragraph content mismatch"
            );

            // Check the cell with italic text
            assert!(
                table.rows[1].cells[1].runs[0].formatting.italic,
                "Text should have italic formatting"
            );
            assert_eq!(
                normalize_test_string(&table.rows[1].cells[1].runs[0].text),
                normalize_test_string("Cursive"),
                "Italic text content mismatch"
            );
        }
...
fn test_parse_empty_table() {
    ...
    match parse_table(&tbl_node) {
        Ok(table) => {
            ...
            assert_eq!(
                table.rows[0].cells[0].runs.len(),
                0,
                "Empty cell should have no runs"
            );
            assert_eq!(
                table.rows[0].cells[1].runs.len(),
                0,
                "Empty cell should have no runs"
            );
            assert_eq!(
                table.rows[1].cells[0].runs.len(),
                0,
                "Empty cell should have no runs"
            );
            ...
            assert_eq!(
                table.rows[1].cells[1].runs.len(),
                1,
                "Cell should have one run"
            );
            assert_eq!(
                normalize_test_string(&table.rows[1].cells[1].runs[0].text),
                normalize_test_string("Only content"),
                "Cell content mismatch"
            );
        }
...
fn test_parse_pic_with_image() {
    ...
    match parse_pic(&pic_node) {
        Ok(image_ref) => {
            ...
            assert_eq!(
                image_ref.target, "",
                "Image target should be empty initially"
            );
        }
...
fn test_parse_slide_xml_reads_graphic_frame_position_from_p_xfrm() {
    ...
    let table = elements
        .iter()
        .find(|element| matches!(element, SlideElement::Table(_, _)))
        .expect("Expected table element");

    match table {
        SlideElement::Table(_, position) => {
            assert_eq!(*position, ElementPosition { x: 111, y: 222 })
        }
...
fn test_parse_slide_xml_applies_group_transform_to_child_positions() {
    ...
    let text = elements
        .iter()
        .find(|element| matches!(element, SlideElement::Text(_, _)))
        .expect("Expected text element");

    match text {
        SlideElement::Text(_, position) => {
            assert_eq!(*position, ElementPosition { x: 2000, y: 5000 })
        }
...
fn test_parse_slide_xml_uses_inherited_placeholder_position() {
    ...
    match text {
        SlideElement::Text(_, position) => assert_eq!(
            *position,
            ElementPosition {
                x: 695326,
                y: 333375
...
fn test_extract_inherited_positions_falls_back_to_master_placeholder() {
    ...
    let master_positions =
        extract_inherited_positions(master_xml.as_bytes(), &InheritedPositions::default())
